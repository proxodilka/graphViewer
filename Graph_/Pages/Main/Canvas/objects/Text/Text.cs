...
namespace Graph_
{
    ...
    public class Text
    {
        ...
        private void initNulls(float? _fontSize, Color? _color)
        {
            color = _color ?? Color.Black;
            fontSize = _fontSize ?? 16;
        }

        public Text(string _text, PointF _center, float? _fontSize, Color? _color)
        {
        ...
        public RectangleF getAsTranslatedRectangle(int scale, PointF center)
        {
            float fontSize = this.fontSize * scale;
            float x = Center.X * scale + center.X;
            float y = center.Y - Center.Y * scale;

            return new RectangleF(x - fontSize * text.Length / 2, y - fontSize / 2, fontSize * text.Length, fontSize);
        }
